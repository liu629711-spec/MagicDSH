/**
 * 会话工作区路径解析。所有 devtools 工具的相对路径都以此为界：
 * 根 = 调用方 agent 的会话工作区（DSH 原生约定 exec.agent.session.header.cwd，
 * 见 reference-project/deepseek-harness/packages/fs/tool-fs/src/session-cwd.ts），
...
export function workspaceRootOf(exec: DevtoolsExec | undefined): string {
...
/** 规范化工作区相对路径：拒空、拒绝对路径、拒 `..` 逃逸；返回去尾斜杠形态。 */
export function resolveInWorkspace(root: string, relRaw: string, field: string): string {
  const rel = relRaw.trim()
  if (rel === '') throw new Error(`${field} 不能为空：请提供工作区相对路径`)
