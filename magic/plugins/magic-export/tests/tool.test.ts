...
import { apply, name, inject, type ExportResult } from '../src/index.ts'
import { LAYOUT_INVALID_MESSAGE } from '../src/layout.ts'
...
function fakeExec(cwd: string): unknown {
  return { agent: { session: { header: { cwd } } } }
}

async function loadTools(): Promise<Map<string, RegisteredTool>> {
  const registered: RegisteredTool[] = []
  await apply({ tools: { register: (tool: RegisteredTool) => { registered.push(tool) } } })
...
test('md_to_docx：工作区内 .md → 兄弟 .docx，manifest 口径对齐蓝本', async () => {
  ...
  const result = await tools.get('md_to_docx')!.execute({ path: '报告.md' }, fakeExec(ws)) as ExportResult
...
test('md_to_pdf：工作区内 .md → 兄弟 .pdf，头部 %PDF，manifest 口径对齐蓝本', async () => {
  ...
  const result = await tools.get('md_to_pdf')!.execute({ path: 'docs/报告.md' }, fakeExec(ws)) as ExportResult
...
test('layout 档位透传：official 导出成功且产物落盘（缩进结构断言在 convert.test.ts）', async () => {
  ...
  const tools = await loadTools()
  const result = await tools.get('md_to_docx')!.execute({ path: '公文.md', layout: 'official' }, fakeExec(ws)) as ExportResult
...
test('安全：越界源 / 越界图片 / 缺源 / 缺工作区根均拒绝', async () => {
  ...
  const result = await docx.execute({ path: 'with-img.md' }, fakeExec(ws)) as ExportResult
