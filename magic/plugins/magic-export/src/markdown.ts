...
export function docxPathForMarkdown(mdPath: string): string {
...
function siblingPathFor(mdPath: string, ext: string): string {
  const p = mdPath.replaceAll('\\', '/').trim()
  const lower = p.toLowerCase()
  if (lower.endsWith('.markdown')) return p.slice(0, -'.markdown'.length) + ext
  if (lower.endsWith('.md')) return p.slice(0, -'.md'.length) + ext
  return `${p}${ext}`
}

/** 收集文档里按出现顺序去重后的图片 src（蓝本 collect_image_srcs）。 */
export function collectImageSrcs(tokens: Token[]): string[] {
  const seen = new Set<string>()
  const out: string[] = []
  for (const inline of iterInlineTokens(tokens)) {
    if (inline.type !== 'image') continue
    const src = String((inline as Tokens.Image).href ?? '').trim()
...
export function* iterInlineTokens(tokens: Token[]): Generator<Token> {
  for (const token of tokens) {
    switch (token.type) {
      case 'paragraph':
      case 'heading':
      case 'text': {
        for (const child of inlineChildrenOf(token)) yield child
        break
      }
      case 'list': {
        for (const item of (token as Tokens.List).items) yield* iterInlineTokens(item.tokens ?? [])
        break
      }
      case 'blockquote': {
        yield* iterInlineTokens((token as Tokens.Blockquote).tokens ?? [])
        break
      }
      case 'table': {
        const table = token as Tokens.Table
        for (const cell of [...table.header, ...table.rows.flat()]) {
          for (const child of cell.tokens ?? []) yield child
        }
        break
      }
      default:
...
export function inlineChildrenOf(token: Token): Token[] {
...
export function isEmbeddableRelativeSrc(src: string): boolean {
  const s = (src ?? '').trim()
...
export function resolveWorkspaceImagePath(mdPath: string, src: string): string | null {
  if (!isEmbeddableRelativeSrc(src)) return null
 ...
 */
export function inlineTextWithImages(
  children: Token[] | undefined | null,
): InlineTextResult {
    ...
    for (const child of children) {
      switch (child.type) {
        case 'text': {
          ...
          else parts.push((child as Tokens.Text).text ?? '')
          break
        }
        case 'escape':
          parts.push((child as Tokens.Text).text ?? '')
          break
        case 'codespan':
          parts.push((child as Tokens.Codespan).text ?? '')
          break
        case 'softbreak':
        case 'br':
          parts.push('\n')
...
export function inlineText(children: Token[] | undefined | null): string {
