...
export const LAYOUT_OFFICIAL = 'official';
export const DOC_LAYOUTS = [LAYOUT_STANDARD, LAYOUT_OFFICIAL];
/** 中文公文通例：正文首行缩进两个字符（随字号走，不是固定磅值）。 */
export const FIRST_LINE_INDENT_CHARS = 2;
/** 工具 schema 复用同一段措辞，两个导出器口径一致。 */
export const LAYOUT_PARAM_DESCRIPTION = '排版档位（可选，默认 standard）：standard=技术文档/报告；'
    + 'official=中文正式文书（正文首行缩进两字）。用户要起诉状、公函、通知、'
    + '声明等可提交的正式文书时传 official，技术文档别传。';
export const LAYOUT_INVALID_MESSAGE = `layout 须为 ${DOC_LAYOUTS.join(' / ')}（留空 = ${LAYOUT_STANDARD}）。`;
/**
