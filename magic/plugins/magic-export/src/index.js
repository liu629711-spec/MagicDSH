...
import { convertMarkdownToDocx } from "./md-to-docx.js";
import { convertMarkdownToPdf } from "./md-to-pdf.js";
import { LAYOUT_INVALID_MESSAGE, LAYOUT_PARAM_DESCRIPTION, DOC_LAYOUTS, LAYOUT_STANDARD, parseLayout, } from "./layout.js";
import { collectImageSrcs, docxPathForMarkdown, isEmbeddableRelativeSrc, parseMarkdown, pdfPathForMarkdown, resolveWorkspaceImagePath, } from "./markdown.js";
import { isInside, resolveInWorkspace, workspaceRootOf } from "./paths.js";
...
function normalizeMdPath(mdPath) {
...
function readMarkdownSource(root, rel) {
    let abs;
    try {
    ...
    if (!stats.isFile())
        throw new Error(`不是文件：${rel}`);
    return readFileSync(abs, 'utf8');
}
/** 输出兄弟文件；目录不存在则创建（蓝本 backend.write_bytes 的落盘语义）。 */
function writeOutput(root, outRel, bytes) {
    const abs = resolveInWorkspace(root, outRel, 'output');
    if (!isInside(root, abs))
        throw new Error('输出路径非法：超出工作区范围');
    mkdirSync(dirname(abs), { recursive: true });
    writeFileSync(abs, bytes);
    return bytes.byteLength;
}
function buildManifest(kind, outRel, sourceRel, bytes, warnings) {
    const lines = [
...
async function runDocxExport(root, args) {
    const layout = parseLayout(args.layout);
    if (layout === null)
        throw new Error(LAYOUT_INVALID_MESSAGE);
...
async function runPdfExport(root, args) {
    const layout = parseLayout(args.layout);
    if (layout === null)
        throw new Error(LAYOUT_INVALID_MESSAGE);
    const rel = normalizeMdPath(args.path);
...
export async function apply(ctx) {
    ...
    ctx.tools.register({
        name: 'md_to_docx',
        description: '把工作区内的 Markdown 文件确定性导出为同目录同名 Word（.docx）。'
        ...
        async execute(args, exec) {
            ...
            return runDocxExport(root, (args ?? {}));
        },
    });
    // ── md_to_pdf：Markdown → 同目录同名 .pdf（蓝本同名工具） ──────────────
    ctx.tools.register({
        name: 'md_to_pdf',
        description: '把工作区内的 Markdown 文件确定性导出为同目录同名 PDF（.pdf）。'
