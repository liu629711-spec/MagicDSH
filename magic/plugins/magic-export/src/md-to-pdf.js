...
import { LAYOUT_OFFICIAL, LAYOUT_STANDARD } from "./layout.js";
import { isCjkChar, inlineText, inlineTextWithImages, parseMarkdown } from "./markdown.js";
const HEADING_PT = { 1: 20, 2: 16, 3: 14, 4: 12 };
const BODY_PT = 11;
const CODE_PT = 9;
const TABLE_PT = 10;
const LABEL_PT = 8;
...
export const CJK_FONT_WARNING = '未找到可用的 CJK 字体（PDF 标准字体仅支持 WinAnsi，未内嵌 CJK 字体）。'
...
// 块级渲染
// ---------------------------------------------------------------------------
function writeHeading(state, fonts, text, level) {
...
function writeParagraph(state, fonts, text, firstLineIndent) {
    if (text.trim() === '') {
        state.y += mm(2);
        return;
    }
...
function writeCode(state, fonts, content, info) {
...
function writeBlockquote(state, fonts, text) {
...
function writeListItem(state, fonts, text, ordered, index, level) {
...
function writeTable(state, fonts, rows) {
    if (rows.length === 0)
        return;
...
export async function convertMarkdownToPdf(markdown, options = {}) {
    const warnings = [CJK_FONT_WARNING];
    const warn = (msg) => {
...
function renderBlocks(state, fonts, tokens, blockCtx, warn) {
    for (const token of tokens) {
        switch (token.type) {
            ...
            case 'paragraph': {
                const { text, warnings } = inlineTextWithImages(token.tokens ?? []);
            ...
            case 'list':
                renderList(state, fonts, token, blockCtx, warn);
                break;
            case 'code':
                writeCode(state, fonts, token.text ?? '', (token.lang ?? '').trim());
                break;
            case 'table': {
                const table = token;
                const rows = [];
            ...
            case 'blockquote': {
                ...
                for (const child of quote.tokens ?? []) {
                    if (child.type === 'paragraph') {
                        const { text, warnings } = inlineTextWithImages(child.tokens ?? []);
                        for (const w of warnings)
                            warn(w);
                        writeBlockquote(state, fonts, text);
                    }
                    else if (child.type === 'list') {
                        renderList(state, fonts, child, blockCtx, warn);
                    }
                }
                break;
            }
...
function renderList(state, fonts, token, blockCtx, warn) {
    let index = 0;
    for (const item of token.items) {
        index += 1;
        for (const child of item.tokens ?? []) {
            if (child.type === 'list') {
                renderList(state, fonts, child, { ...blockCtx, level: blockCtx.level + 1 }, warn);
                continue;
            }
...
function inlineChildrenCompat(token) {
